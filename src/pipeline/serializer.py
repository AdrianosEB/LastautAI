...
from src.graph.topological import topological_sort
from src.pipeline.planner import PlanResult
...
def serialize(plan_result: PlanResult, output_format: str = "json") -> dict | str:
    """
    Walk the DAG in topological order and produce the workflow definition.
    Returns a dict for JSON, or a YAML string for YAML.
    """
    ...
    for step_id in ordered_ids:
        ...
        # Check for conditional edges targeting this step
...
def _generate_name(plan_result: PlanResult) -> str:
    """Generate a readable name from the first step's description."""
